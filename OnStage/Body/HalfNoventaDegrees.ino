...
void halfNoventaDegrees() {
  ...
  while (stepperL.distanceToGo() != 0 && stepperR.distanceToGo() != 0) {
    ...
    //In this case(and also taking in consideradion the inertia effect on the movement), this code will make it stop whenver it rotates 45 degrees
    if (((int)(euler.x() - reference + 360) % 360) > 5 && ((int)(euler.x() - reference + 360) % 360) <= 317) {
      break;
    }
  }
  delay(1000);
